        ...
        {!modules ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {modules.map((m) => {
              ...
              return (
                <Link key={m._id} href={`/dashboard/student/training/${m._id}`}>
                  <Card className="p-5 h-full flex flex-col hover:border-accent/40 transition-colors">
                    <div className="flex items-start justify-between gap-2">
                      <Badge tone={meta.tone}>{meta.label}</Badge>
                      <Icon size={17} className={meta.tone === "success" ? "text-accent" : "text-text-muted"} />
                    </div>
                    <h3 className="font-display font-semibold text-text-main mt-3">{m.title}</h3>
                    <p className="text-sm text-text-muted mt-1.5 line-clamp-2 flex-1">{m.summary}</p>
                    <div className="flex items-center gap-1.5 text-xs text-text-muted mt-4 pt-4 border-t border-border">
                      <Clock size={12} />
                      {m.estimatedMinutes} min · {m.category}
                      {m.progress?.score != null && (
                        <span className="ml-auto font-mono-data font-medium text-text-main">{m.progress.score}%</span>
                      )}
                    </div>
                  </Card>
                </Link>
